 ...
 * 도메인의 컨버터는 아무도 확인하지 않는다. 여기서 클래스패스를 뒤져 <b>모든</b> 컨버터의
 * <b>모든</b> enum 상수를 왕복시킨다.
 *
...
class EnumConverterContractTest {
    ...
    @Test
    @DisplayName("컨버터는 전부 공통 베이스를 상속한다 — 변환 규칙이 한 곳에만 있어야 한다")
    void everyConverterExtendsCommonBase() {
        var provider = new ClassPathScanningCandidateComponentProvider(false);
        provider.addIncludeFilter(new AssignableTypeFilter(AttributeConverter.class));

        Set<BeanDefinition> all = provider.findCandidateComponents(BASE_PACKAGE);
