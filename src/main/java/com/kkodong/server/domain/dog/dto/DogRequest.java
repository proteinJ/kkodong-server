...
public class DogRequest {
    ...
    /**
     * 성향 태그는 최대 3개(빈 배열 허용). 값 유효성은 {@code kkodong.dog.personality.tags}
     * 설정이, 개수는 여기 @Size와 DB CHECK 제약이 이중으로 막는다.
    ...
    public record registration(
            ...
            @Schema(description = "성향 태그 (최대 3개)", example = "[\"활발함\",\"사교적\"]")
    ...
    public record patch(
            ...
            @Schema(description = "성향 태그 (최대 3개)", example = "[\"차분함\"]")
            @Size(max = 3, message = "성향 태그는 최대 3개까지 선택할 수 있습니다.")
            List<String> personalityTraits,
