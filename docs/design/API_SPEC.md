...
> **2026-09-13 보강 (서버 착수, #50)** — 이 절에 비어 있던 값을 서버 쪽에서 정해 채웠다.
> **기존 필드 이름·모양은 바꾸지 않았다.** 앱 동작에 영향이 있는 것은 네 가지다 —
> 반경 상한과 페이지 크기·정렬(14.2 파라미터), `isOpenNow: null`(14.2 응답),
> 상세의 선택 위치 파라미터와 404(14.3).
...
| `radiusKm` | | 기본 `3`, **최대 `10`** | 반경. 10 을 넘으면 **10 으로 잘라서** 조회한다(에러 아님) |
| `categories` | | `kindergarten,grooming` | 쉼표 구분. **비어 있으면 전 업종** |
| `keyword` | | `미용` | 상호 부분 일치 |
| `cursor` | | | 페이징. 첫 요청은 비우고, 이후엔 응답의 `nextCursor` 를 그대로 넣는다 |
...
- **한 번에 최대 50개**를 **가까운 순**(거리가 같으면 `id` 순)으로 내려준다. 더 있으면
  `nextCursor` 가 채워지고, 마지막 페이지면 `null` 이다.
- **다음 페이지는 첫 요청과 같은 `lat`·`lng`·`radiusKm`·`categories`·`keyword` 로 부른다.**
  조건이 바뀌면 커서가 가리키는 순서도 달라져 중복·누락이 생긴다. 위치나 필터가
  바뀌었으면 `cursor` 없이 처음부터 다시 부른다.
- `lat` 은 −90~90, `lng` 는 −180~180, `radiusKm` 는 0 보다 커야 한다. 벗어나면 400(`CM001`).
...
- **`isOpenNow`는 서버가 판정한다.** 클라이언트가 `businessHours`로 다시 계산하면
  ...
  판정 규칙은 아래와 같다.
  - 기준 시각은 **한국 시간**(`Asia/Seoul`)이다. `merchants` 에 매장 시간대 칸이 없다.
  - **`businessHours` 가 비어 있으면 `null`** 이다. 모르는 것을 "닫힘"으로 보이지 않는다.
    형식이 잘못된 줄(요일 오타, `HH:mm` 이 아님)은 건너뛰고, 남는 줄이 없어도 `null` 이다.
  - **오늘이 `closedDates` 에 있으면 오늘 시작하는 영업은 없는 것**으로 본다.
    전날 밤부터 이어지는 새벽 영업은 아래 자정 규칙대로 전날 기준이다.
  - **같은 요일에 여러 줄**(점심 휴게 등)을 허용한다. 한 줄이라도 지금을 포함하면 영업 중이다.
  - **`close` 가 `open` 보다 이르거나 같으면 자정을 넘기는 영업**으로 본다. 예를 들어
    `sat 22:00~02:00` 이면 일요일 01:30 도 영업 중이고, `00:00~00:00` 은 24시간 영업이다.
    자정을 넘긴 새벽 시간은 **전날 줄과 전날의 `closedDates`** 로 판단한다.
    `close` 의 `24:00` 은 그날 자정으로 읽는다(`09:00~24:00` 은 자정 직전까지 영업).
  - 위 어느 줄에도 지금이 들어가지 않으면 `false` 다.
- **`friendDogCount`가 이 화면의 유일한 차별점이다**(요구사항 `C-02`).
  ...
  지금은 친구 맺기(`FRIEND-2`)가 없어 **항상 0** 이다. 요구사항서의 "공개에 동의한 친구만
  센다"(`FR-C01-04`)는 `UD-F` 가 정해지면 반영한다 — 지금은 동의를 저장할 곳이 없다.
- **`thumbnailUrl`은 `imageUrls`의 첫 장**이다. 목록에 배열 전체를 넣지 않는다 —
  핀 수십 개에 사진 배열이 딸려 오면 응답이 폭발한다. 사진이 없으면 `null` 이다.
- **`address` 는 상세(14.3)와 같은 전체 주소**다. 위 예시의 `"서울 마포구"` 는 줄여 적은
  것일 뿐이다 — `merchants` 에 주소 칸이 하나뿐이다.
...
- `distanceKm`는 **요청자 위치를 모르면 `null`**로 둔다. 상세는 위치 없이도 열려야 한다.
  위치는 **선택 쿼리 파라미터** `?lat=&lng=` 로 받는다. 둘 다 와야 계산하고,
  한쪽만 오거나 범위를 벗어나면 400(`CM001`)이다.
- `isOpenNow` · `friendDogCount` 는 14.2 와 같은 규칙으로 계산한다.
- **매장이 없거나 `ACTIVE` 가 아니면 404(`PL001`)** 다. 둘을 구분하지 않는다 —
  대기·정지 매장이 존재한다는 사실을 견주에게 드러낼 이유가 없다.
